...
bitmap *bitmap_expand(struct bitmap *bitmap,int size)
{
	...
	new_bm = malloc(sizeof *new_bm);

	if( !bitmap || !new_bm)
		return NULL;

	size_origin = bitmap_size( bitmap );
	
	new_bm->bit_cnt = size_origin + size;

	new_bm->bits = realloc(bitmap->bits, byte_cnt(new_bm->bit_cnt));

	if(!new_bm->bits)
		return NULL;

	bitmap_set_multiple(new_bm, size_origin, size, false);
