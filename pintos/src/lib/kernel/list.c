...
void list_swap(struct list_elem *a,struct list_elem *b)
{
	...
	if( a->next == b )
	{
		struct list_elem *prev_a,*next_b;
	...
	{
		struct list_elem *prev_b,*next_a;
...
void list_shuffle(struct list *list)
{
	...
	list_cnt = list_size(list);

	for( i = 0 ; i < list_cnt + 5 ;++i)
	{
		struct list_elem *el1 , *el2;
		el1 = list_get_nth(list, rand() % list_cnt);
		el2 = list_get_nth(list, rand() % list_cnt);
...
/*
 *	Get nth list element from list. 
 */
struct list_elem*
list_get_nth(struct list *list,size_t n)
{
	struct list_elem *srch;
	size_t i;

	srch = list_begin(list);

	for(i = 0 ;
			i < n && srch != list_end(list); 
			++i , srch = list_next(srch)  )
	{
		;
	}
